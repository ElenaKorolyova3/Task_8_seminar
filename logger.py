...
def put_data():
    ...
    if number_file == 1:  # Можно сделать нумерацию внутри файла
        ...
        num_element = int(input("Что Вы хотите изменить: 1) Имя 2) Фамилию 3) Номер телефона 4) Город 5) Всю запись"))
        ...
        if  num_element == 1:
            data_second = data_second[:number_journal] + [f'{name_data()};{data_temp[1]};{data_temp[2]};{data_temp[3]}\n'] + \
                          data_second[number_journal + 1:]
        elif num_element == 2:
            data_second = data_second[:number_journal] + [f'{data_temp[0]};{surname_data()};{data_temp[2]};{data_temp[3]}\n'] + \
                          data_second[number_journal + 1:]
        elif num_element == 3:
            data_second = data_second[:number_journal] + [f'{data_temp[0]};{data_temp[1]};{phone_data()};{data_temp[3]}\n'] + \
                          data_second[number_journal + 1:]
        elif num_element == 4:
...
def delete_data():
    ...
    if number_file == 1:  # Можно сделать нумерацию внутри файла
        ...
        data_temp = data_first[number_journal].split()
        num_element = int(input("Что Вы хотите удалить: 1) Имя 2) Фамилию 3) Номер телефона 4)Город 5) Всю запись"))

        if num_element == 1:
            data_first = data_first[:number_journal] + [f'{""}\n{data_temp[1]}\n{data_temp[2]}\n{data_temp[3]}\n'] + \
                         data_first[number_journal + 1:]
        elif num_element == 2:
            data_first = data_first[:number_journal] + [f'{data_temp[0]}\n{""}\n{data_temp[2]}\n{data_temp[3]}\n'] + \
                         data_first[number_journal + 1:]
        ...
        if num_element == 1:
            data_second = data_second[:number_journal-1] + [f'{""};{data_temp[1]};{data_temp[2]};{data_temp[3]}'] + \
                          data_second[number_journal:]
        elif num_element == 2:
            data_second = data_second[:number_journal-1] + [f'{data_temp[0]};{""};{data_temp[2]};{data_temp[3]}\n'] + \
                          data_second[number_journal:]
        elif num_element == 3:
            data_second = data_second[:number_journal-1] + [f'{data_temp[0]};{data_temp[1]};{""};{data_temp[3]}\n'] + \
                          data_second[number_journal:]
        elif num_element == 4:
            data_second = data_second[:number_journal-1] + [f'{data_temp[0]};{data_temp[1]};{data_temp[2]};{""}\n'] + \
                          data_second[number_journal:]
        elif num_element == 5:
